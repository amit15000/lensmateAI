...
export function GearInput() {
  const { gear, setGear, uploadedFile, analyzeScene, isAnalyzing } =
    useProjectStore();
  ...
  const [newLensSubtitle, setNewLensSubtitle] = useState("");
  ...
  const handleAddLens = () => {
    ...
    setLenses([
      ...lenses,
      { name: newLensName, subtitle: newLensSubtitle || "Custom Lens" },
    ]);
    setNewLensName("");
    setNewLensSubtitle("");
    setAddingLens(false);
    ...
    <div className="space-y-6 mt-8">
      ...
      <div className="space-y-2">
        ...
        {!addingLens ? (
          <Button
            variant="outline"
            className="w-full bg-gray-700 border-gray-600 hover:bg-gray-600 border-dashed mt-2"
            onClick={() => setAddingLens(true)}
          >
            <Plus className="w-4 h-4 mr-2" /> Add Lens
          </Button>
        ) : (
          <div className="space-y-2 mt-2">
            <Input
              placeholder="Lens name (e.g., 35mm)"
              value={newLensName}
              onChange={(e) => setNewLensName(e.target.value)}
              className="bg-gray-700 border-gray-600"
            />
            <Input
              placeholder="Subtitle (e.g., f/1.8 Prime)"
              value={newLensSubtitle}
              onChange={(e) => setNewLensSubtitle(e.target.value)}
              className="bg-gray-700 border-gray-600"
            />
            <Button
